...
static int GetWord( BYTE * BufFile, int Pos, int Deprotect )
{
...
//
static int AddWord( BYTE * BufFile, int Pos, char * Listing, int Deprotect )
{
          ...
          )
{
    ...
    * Listing = 0;
    ...
    for ( ; strlen( Listing ) < MAX_FILESIZE - 16 && Pos < Longueur; )
        {
        if ( IsBasic )
            {
        ...
        do
            ...
            if ( DansChaine || ! IsBasic || DansComment )
                {
                Tmp[ 0 ] = ( char )Token;
                Tmp[ 1 ] = 0;
                strcat( Listing, Tmp );
                ...
                if ( Token > 0x7F && Token < 0xFF )
                    {
                    // #### Traitement particulier du ':' avant le ELSE ou le '
                    if (  Listing[ strlen( Listing ) - 1 ] == ':' 
                       && ( Token == 0x97 || Token == 0xC0 )
                       )
                        {
                        Listing[ strlen( Listing ) - 1 ] = 0;
                        DansComment = ( Token == 0xC0 );
                        }

                    strcat( Listing, MotsClefs[ Token & 0x7F ] );
                    }
                else
                    if ( Token >= 0x0E && Token <= 0x18 )
                        strcat( Listing, Nbre[ Token - 0x0E ] );
                    else
                        if ( Token >= 0x20 && Token < 0x7C )
                            ...
                            switch( Token )
                                ...
                                case 0x1F : // Constante flottante
                                    ...
                                    Pos += 5;
                                    sprintf( Tmp, "%f", f * pow( 2, ( double )exp ) );
    ...
    if ( Langue == LOC_FRANCAIS )
        {
        for ( int i = strlen( Listing); i--; )
            if ( Listing[ i ] > 32 && Listing[ i ] < 127 )
                Listing[ i ] = ConvCpcFr[ Listing[ i ] ];
        }
}
