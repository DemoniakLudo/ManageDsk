...
// Formater une piste
//
static void FormatTrack(int nDSK, CPCEMUEnt* Infos, int t, int MinSect, int NbSect) {
	CPCEMUTrack* tr = (CPCEMUTrack*)&ImgDsk[nDSK][sizeof(CPCEMUEnt) + t * Infos->TrackSize];
	memset(&ImgDsk[nDSK][sizeof(CPCEMUEnt) + sizeof(CPCEMUTrack) + (t * Infos->TrackSize)], 0xE5, SECTSIZE * NbSect);
...
void InterlaceTrack(int nDSK, int t) {
	...
	for (s = 0; s < tr->NbSect; s++)
		memcpy(&RawSect[s * SECTSIZE], ReadSect(nDSK, t, s, FALSE), SECTSIZE);
...
void LowLevelFormatTrack(int nDSK, int t, int NumSect, int NbSect, BYTE C, BYTE H, BYTE R, BYTE N) {
...
void SetInfoDirEntry(int nDSK, int NumDir, StDirEntry* Dir) {
	...
	memcpy(&ImgDsk[nDSK][((NumDir & 15) << 5) + GetPosData(nDSK, t, s, TRUE)], Dir, sizeof(StDirEntry));
}
...
StDirEntry* GetInfoDirEntry(int nDSK, int NumDir) {
	...
	memcpy(&Dir, &ImgDsk[nDSK][((NumDir & 15) << 5) + GetPosData(nDSK, t, s, TRUE)], sizeof(StDirEntry));
	return(&Dir);
}
...
BOOL IsValidDsk(char* NomFic) {
	...
	if (fp != INVALID_HANDLE_VALUE) {
		...
		if (!strncmp(Infos.debut, "MV -", 4) || !strncmp(Infos.debut, "EXTENDED CPC DSK", 16))
			Ret = TRUE;
...
BOOL WriteDsk(int nDSK, char* NomDsk) {
	...
	HANDLE fp = CreateFile(NomDsk, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, 0, NULL);
	if (fp != INVALID_HANDLE_VALUE) {
		if (!Infos->TrackSize)
