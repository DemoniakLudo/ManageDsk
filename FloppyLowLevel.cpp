...
CFloppyLowLevel::CFloppyLowLevel( int n, int t, int m, CWnd * pParent /*=NULL*/)
    : CDialog(CFloppyLowLevel::IDD, pParent)
{
...
{
    DWORD v = 0;
    HANDLE h = CreateFile( "\\\\.\\fdrawcmd"
                         , GENERIC_READ | GENERIC_WRITE
                         , 0, NULL, OPEN_EXISTING, 0, NULL
                         );
    if ( h != INVALID_HANDLE_VALUE )
        {
        DeviceIoControl( h, IOCTL_FDRAWCMD_GET_VERSION
                       , NULL, 0, &v, sizeof( v )
                       , &rt, NULL
                       );
        CloseHandle( h );
...
HANDLE CFloppyLowLevel::InitDriver( int Drive, BOOL CheckMedia )
{
    ...
    HandleDriver = CreateFile( szDevice
                             , GENERIC_READ | GENERIC_WRITE
                             , 0, NULL, OPEN_EXISTING, 0, NULL
                             );
    if ( HandleDriver != INVALID_HANDLE_VALUE )
        ...
        if ( ! DeviceIoControl( HandleDriver, IOCTL_FD_SET_DISK_CHECK
                              , &b, sizeof( b ), NULL, 0
                              , &rt, NULL
                              )
           )
...
BOOL CFloppyLowLevel::GetGeometry( int Drive )
{
    ...
    HANDLE h = CreateFile( szDevice
                         , GENERIC_READ | GENERIC_WRITE
                         , FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL
                         );
    if ( h != INVALID_HANDLE_VALUE )
        {
        if ( ! DeviceIoControl( h, IOCTL_DISK_GET_MEDIA_TYPES
                              , NULL, 0, &Dsk, sizeof( Dsk )
                              , &rt, NULL
                              )
           )
            Erreur();
...
{
    return( DeviceIoControl( HandleDriver, IOCTL_FD_GET_FDC_INFO
...
{
    return( DeviceIoControl( HandleDriver, IOCTL_FDCMD_SEEK
                           , &Piste, sizeof( Piste ), NULL, 0
                           , &rt, NULL
                           )
          );
}
...
BOOL CFloppyLowLevel::ReadFloppyLayout( int Piste )
{
    ...
    sp.head = ( BYTE )Tete;
    // Lecture infos piste
    if ( DeviceIoControl( HandleDriver, IOCTL_FD_SCAN_TRACK
                        , &sp, sizeof( sp ), psr, sizeof( buf )
                        , &rt, NULL
                        )
       )
        {
...
BOOL CFloppyLowLevel::ReadTrack( BYTE Piste, CPCEMUTrack * Trk )
{
    ...
    for ( int s = 0; s < Trk->NbSect && ! Break; s++ )
        ...
        for ( int retry = 0; retry < 5; retry++ )
            {
            // Lecture secteur
            b = DeviceIoControl( HandleDriver, IOCTL_FDCMD_READ_DATA
                               , &rwp, sizeof( rwp ), Datas, SectSize
                               , &rt, NULL
                               );
            if ( b )
                break;
...
BOOL CFloppyLowLevel::WriteTrack( BYTE Piste, CPCEMUTrack * Trk )
{
    ...
    for ( int s = 0; s < Trk->NbSect && ! Break; s++ )
        ...
        if ( ! NoData )
            ...
            // Ecriture secteur
            b = DeviceIoControl( HandleDriver, IOCTL_FDCMD_WRITE_DATA
                               , &rwp, sizeof( rwp ), Datas, SectSize
                               , &rt, NULL
                               );
            }
...
//
BOOL CFloppyLowLevel::FormatTrack( BYTE Piste, CPCEMUTrack * Trk )
{
    ...
    return( DeviceIoControl( HandleDriver, IOCTL_FDCMD_FORMAT_TRACK
                           , pfp, ( PBYTE )ph - abFormat, NULL, 0
                           , &rt, NULL
                           )
          );
}
