...
//
void Desass( BYTE * Prg, char * Listing, int StartAdr, int Longueur )
{
    int i, Instr, Inst2 = 0, Inst3 = 0, Inst4 = 0, Ad16;
    char * Chaine, *p;
    static char Inst[ 1024 ];
    static char Ligne[ 1024 ];
    static char TmpAdr[ 1024 ];
    static char Tmp[ 128 ];
    int Adr, OldAdr, PosD = 0;
    char Ad8;

    * Listing = 0;
    for ( Adr = 0; Adr < Longueur && PosD < MAX_FILESIZE - 16; )
        {
        OldAdr = Adr;
        Instr = Prg[ Adr++ ];
        ...
        Hex( &Listing[ PosD ], StartAdr + OldAdr, 4 );
        Listing[ PosD + 4 ] = ' ';
        PosD += 5;
